...
/*//Find Organization given a Name(Partial or Full) (Returns an Array)
export function findOrgByNamePartial(req, res) {
  ...
  return User.find({name:  { $regex: searchName, $options: 'i'}, role: 'Organisation'})
    .exec()
    .then((orgz) => {
        if(orgz != null && orgz.length != 0) {
            console.log("Found Organisation : " + orgz);
            //res.send(org);
            var orgList = [];
            for(var i = 0; i<orgz.length; ++i) {
              var tOrg = { name: orgz.name, _id: orgz._id };
              orgList.push(tOrg);
            }
...
export function getOrgList(req, res) {
    User.find( {role: 'Organisation'}, 'name _id', function(err, orgList) {
      if(err) {
        console.error(err);
        res.send(err);
      }
      else {
        console.log('Organisations List : ', orgList);
        res.send(orgList);
      }
    });
}